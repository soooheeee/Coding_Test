...
# 핵심 아이디어 및 기능
1. 배열로 미리 값을 다 입력해두기 (킹,퀸,룩,비숍,나이트,폰의 개수)
chess=[1,1,2,2,2,8] 
2. 리스트로 값을 입력받기
예) 1 2 3 4 5 6 을 입력하면, num_list는 [1,2,3,4,5,6]이렇게 입력받는다.
num_list = list(map(int,input().split()))
만약에 위에 코드에서 int를 없애면 문자열로 리스트가 입력된다.
num_list = list(map(input().split()))
1 2 3 을 입력하면 ['1', '2', '3']이렇게 입력받는다.
